...
const MAX_LEFT_CONTEXT_CHARACTERS: usize = 128;
...
impl SessionHistory {
    ...
    pub(crate) fn set_external_contexts(&mut self, left_surface: &str, right_surface: &str) {
        ...
        if !left_surface.is_empty() {
            self.left_surface = Some(bounded_context_suffix(left_surface));
        }
        if !right_surface.is_empty() {
            self.external_right_surface = Some(
                right_surface
                    .chars()
                    .take(MAX_LEFT_CONTEXT_CHARACTERS)
                    .collect(),
            );
        }
    }
    ...
    pub(crate) fn record_transient_surface(&mut self, surface: &str) {
        self.append_surface(surface);
        self.previous_commit = None;
    }
    ...
    fn append_surface(&mut self, surface: &str) {
        if surface.is_empty() {
            return;
        }
        let mut context = self.left_surface.take().unwrap_or_default();
        context.push_str(surface);
        self.left_surface = Some(bounded_context_suffix(&context));
    }
}

fn bounded_context_suffix(surface: &str) -> String {
    let start = surface
        .char_indices()
        .rev()
        .nth(MAX_LEFT_CONTEXT_CHARACTERS - 1)
        .map_or(0, |(index, _)| index);
    surface[start..].to_owned()
}

#[cfg(test)]
mod tests {
    use super::{MAX_LEFT_CONTEXT_CHARACTERS, SessionHistory};
    ...
    fn short_word_can_anchor_the_next_conversion() {
        ...
        assert_eq!(history.previous_surface(), Some("部屋家"));
    }

    #[test]
    fn low_value_commit_breaks_learning_edge_but_remains_model_context() {
        let mut history = SessionHistory::default();
    ...
    #[test]
    fn transient_surface_extends_model_context_without_becoming_a_learning_anchor() {
        let mut history = SessionHistory::default();
        history.record_commit("ぶんしょう", "文章");
        history.record_transient_surface("。plain text");

        assert_eq!(history.previous_surface(), Some("文章。plain text"));
        assert!(history.previous_commit().is_none());
    }
    ...
    fn external_right_context_is_bounded_from_the_caret() {
        let mut history = SessionHistory::default();
        let left = "前".repeat(MAX_LEFT_CONTEXT_CHARACTERS + 10);
        let right = "後".repeat(MAX_LEFT_CONTEXT_CHARACTERS + 10);
