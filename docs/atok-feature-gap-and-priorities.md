...
並列句と同じく、既存Mozc辞書の完全な語の組み合わせを右文脈の証拠にする。一般名詞POS、3〜8文字、漢字列を1個の`の`で結ぶ、辞書cost 7,500以下、右側の語境界が確認できる、という条件をすべて満たす場合だけ既存候補を昇格する。候補生成は行わず、長い名詞の途中一致も使わない。

GSD trainで`胃の調子`、`素の状態`、`真の目的`、`未知の世界`の4件をtop-1へ修正し、悪化は0件だった。GSD dev/test、AJIMEE、JWTD、PUDは候補配列まで不変だった。逆引きartifactの増分は約70 KBで、50,000回×3の変換測定は左文脈のみの経路と同じ変動帯だった。詳細は[evaluation.md](evaluation.md)に記録する。
