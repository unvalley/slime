...
ATOKが確定表記の単純な優先より自然な文章を重視し、azooKey/Zenzaiが左右文脈を別tagで扱う
設計を参考に、モデルへ渡す確定済み左文脈を適応させる。末尾40文字内の最後の句読点より後が
28文字以上ある場合だけ、その十分長い局所節を使う。短い節は話題語を失わないよう40文字を維持する。

12文字境界で発生した`写っ→映っ`の回帰を根拠に28文字へ固定した。公式v3.2-smallの製品比較は
PUD 356→357で`インターフェイスがかけ→インターフェイスが欠け`を修正し、GSD dev/test、
JWTD、AJIMEEは全出力不変だった。5データセット1,689件中の発火は17件、既存正解の悪化は0件。
候補集合や辞書規則を変えず、確定文を意味選択へ渡すprompt境界だけを改善する。詳細は
[evaluation.md](evaluation.md)に記録する。
