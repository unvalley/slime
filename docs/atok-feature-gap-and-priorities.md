...
1. **文脈順位モデルの商用同梱判断が未完了。** 現在の評価モデルはCC BY-SA 4.0で、商用利用そのものは禁止されていない。ただし配布時の帰属・変更表示・ShareAlike対応と、アプリ本体から分離した配布境界の法務確認が必要である。さらにQ5はサイズとtail latency、Q4は複数domainの精度gateを満たさない。辞書規則だけでは意味的な同音異義語を十分に解けない。
...
5. **候補recallは語種ごとに穴が残る。** 候補末尾到達後に短い読みはN-best 32、長い読みは16、一般複合語と固定文節、人名は専用のbounded探索を行う。加えてモデル準備済みの`high-accuracy`だけ、9文字以上の内部再採点候補を32まで広げる。初回表示を重くせず回収範囲を広げたが、組織名・地名・未知の固有名詞は引き続き評価が必要である。
...
2. **ニューラルrerankerは現在No-Go。** 評価上の品質改善はあるが、既存の22.5M teacherは遅延・サイズ条件を満たさず、Q4_K_MはQ5_K_Mより複数domainでtop-1を落とした。蒸留studentも複数held-out非悪化のgateを通っていない。
