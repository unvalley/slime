...
[ATOKの変換強度](https://atok.com/info/features/engine.html)が示す「直前に選んだ語より自然な文章を優先する」境界を、Slimeの既存の定着/一時履歴へ追加した。周辺文脈による辞書順位と文脈なし順位の1位が異なり、5回未満の単発履歴が現在の辞書候補集合にも存在するときだけ、文脈1位を単発履歴より前へ置く。履歴候補自体は候補一覧に残す。

5回以上の定着履歴、同じ直前文脈で2回以上選んだ履歴、ユーザー辞書、現在の辞書候補集合にない履歴は従来順位を維持する。これにより、誤選択を1回学習済みの状態でも`再帰的`、`真価が問われる`、`家宅捜索`、`石油化学`、`渡した`の5種の辞書文脈規則が働く一方、同じ表記を5回使った場合はユーザーの定着した選択を維持する。
...
Slimeは従来、履歴候補が1件でもあれば外部モデルへの候補要求全体を止めていた。このため通常辞書にもある表記を一度選んだだけで、その後に自然な文脈が来てもニューラル順位付けを利用できなかった。変更後は、使用5回未満の一時履歴が通常辞書候補にも存在する場合だけモデルとの比較を許可する。モデルは辞書候補の位置だけを並べ替え、一時履歴の表記も候補内へ残す。辞書外の独自履歴は採点対象に入らないため先頭を維持する。5回以上の確立済み履歴、文脈学習、明示的なユーザー辞書、インストール済みcontext rule、英語逆引き、入力ミス訂正は従来どおりモデルより優先する。
