...
凍結後のheld-outではAJIMEEが0.6950から0.7150へ改善した一方、GSD testは0.8700から0.8669へ1件悪化した。ドメインをまたぐ非劣化条件を満たさないため8候補化は製品へ入れず、上限5候補を維持する。prefix trieは候補scoreと順序を変えない実装なので、重複計算の削減だけを採用する。
