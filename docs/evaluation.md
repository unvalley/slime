...
並列句と同じ逆引き経路を、一般名詞どうしを`の`で結ぶ辞書句へ限定して拡張した。対象は一般名詞POSで始まり終わる3〜8文字、漢字列の間に`の`が1個だけある、辞書cost 7,500以下の句である。右文脈が辞書句と一致し、その直後が漢字・カタカナでない語境界の場合だけ、既存候補を最大3,500 cost昇格する。候補を新しく生成せず、`未知の世界観`の途中を`未知の世界`として扱う前方一致も棄却する。
...
| --- | ---: | ---: | ---: | --- |
| GSD train (1,940) | 0.7036 | **0.7057** | 7件（top-1は4件） | +4 / -0 |
| GSD dev (331) | 0.8640 | 0.8640 | 0件 | 不変 |
| GSD test (323、診断済み) | 0.8607 | 0.8607 | 0件 | 不変 |
| AJIMEE (200) | 0.5300 | 0.5300 | 0件 | 不変 |
| JWTD dev (400) | 0.2950 | 0.2950 | 0件 | 不変 |
| PUD phrase (446、cross-encoder判定に使用済み) | 0.6547 | 0.6547 | 0件 | 不変 |

GSD trainでは`胃の調子`、`素の状態`、`真の目的`、`未知の世界`をtop-1へ修正した。逆引きartifact 3ファイルの合計は7,954,251 bytesから8,024,131 bytesへ69,880 bytes増えた。同一プロセス50,000回を3回測定すると、右文脈ありは0.126〜0.142 ms/op、同じ読み・左文脈だけは0.121〜0.153 ms/opで、追加処理は測定変動内だった。
